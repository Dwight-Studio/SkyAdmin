...
import static fr.dwightstudio.skyadmin.Report.getAssignedReports;
...
public class CommandReports implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender csender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (Report.getAssignedReports().size() == 0) {
                reportsString2.addExtra(" ");
                TextComponent reportText = new TextComponent("Aucun signalement.");
                reportsString2.addExtra(reportText);
            } else {
                for (Report report : Report.getAssignedReports()) {
                    reportsString2.addExtra(" ");
                    TextComponent reportText = new TextComponent("[" + report.getId() + "]");
                    reportText.setColor(ChatColor.AQUA);
                    reportText.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/reports " + report.getId() + " "));
            ...
            if (Report.getAssignedReports(sender).size() == 0) {
                reportsString3.addExtra(" ");
                TextComponent reportText = new TextComponent("Aucun signalement.");
                reportsString3.addExtra(reportText);
            } else {
                for (Report report : Report.getAssignedReports(sender)) {
                    reportsString3.addExtra(" ");
                    TextComponent reportText = new TextComponent("[" + report.getId() + "]");
                    reportText.setColor(ChatColor.AQUA);
                    reportText.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/reports " + report.getId() + " "));
    ...
    public static void notifyReport() {
        for (Player pl : Bukkit.getServer().getOnlinePlayers()) {
            if (pl.hasPermission("skyplugins.reports")) {
                if (Report.getUnassignedReports().size() == 1) {
                    pl.sendMessage(messagePrefix + org.bukkit.ChatColor.AQUA + Report.getUnassignedReports().size() + org.bukkit.ChatColor.RESET + " signalement est en attente de traitement.");
                    pl.playSound(pl.getLocation(), Sound.BLOCK_NOTE_BLOCK_XYLOPHONE, 1, 1);
                } else if (Report.getUnassignedReports().size() > 1) {
                    pl.sendMessage(messagePrefix + org.bukkit.ChatColor.AQUA + Report.getUnassignedReports().size() + org.bukkit.ChatColor.RESET + " signalements sont en attente de traitement.");
                    pl.playSound(pl.getLocation(), Sound.BLOCK_NOTE_BLOCK_XYLOPHONE, 1, 1);
                }
            }
        }
